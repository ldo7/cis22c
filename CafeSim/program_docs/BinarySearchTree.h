/*
 Cafe Simulator
 Written By: Mariia
 Summary: Used to hold list of drink names (key)
 */
...
template<class ItemType>
bool BinarySearchTree<ItemType>::search(const ItemType& anEntry,
...
//recursively remove a node with the specified target value from the binary search tree
template<class ItemType>
...
// It handles three cases based on the children of the target node
template<class ItemType>
...
//Remove the leftmost node from a subtree. If the leftmost node has a right child, it is replaced by its right child
template<class ItemType>
